...
<body style="font-family: sans-serif">
    <div class="user-project" id="app">
        <form id="project-form" action="{{ url('/design/' . $design->id) }}" method="POST" class="d-flex flex-column h-100"
            @keydown.enter="preventFormSubmit" enctype="multipart/form-data">
            @csrf
            @method('put')
    ...
    <script>
        const app = Vue.createApp({
            ...
            created() {
                this.design = JSON.parse(this.design);
                this.design.forEach(element => {
                    this.figuras.push(new Figura(element.type));
                    this.figuras[this.figuras.length - 1].id = element.id;
                    this.figuras[this.figuras.length - 1].updateRelleno(element.fill_r, element.fill_g,
                        element.fill_b, element.fill_a);
                    this.figuras[this.figuras.length - 1].updateGrosor(element.thickness);
                    if (element.type === 'line') {
                        this.figuras[this.figuras.length - 1].updateLinea(element.x1, element.y1, element
                            .x2, element.y2);
                    } else if (element.type !== 'line') {
                        this.figuras[this.figuras.length - 1].updateBorde(element.border_r, element
                            .border_g, element.border_b, element.border_a);
                        this.figuras[this.figuras.length - 1].updateMedidas(element.x, element.y, element.w,
                            element.h);
                        if (element.type === 'text') {
                            this.figuras[this.figuras.length - 1].updateTexto(element.size, element.text);
                        }
                    }
                });
            },
            ...
            mounted() {
                ...
                new p5((p) => {
                            ...
                            if (this.figuras[i].hidden == false) {
                                if (this.figuras[i].selected == true) {
                                    ...
                                    if (this.figuras[i].type === "text") {
                                        p.rect(this.figuras[i].x - 2, this.figuras[i].y - (this.figuras[
                                            i].h), this.figuras[i].w + 4, this.figuras[i].h + 8);
                                        ...
                                        if (this.figuras[i].w >= 0 && this.figuras[i].h >= 0) {
                                            p.rect(this.figuras[i].x - 6, this.figuras[i].y - 6, this
                                                .figuras[i].w + 12, this.figuras[i].h + 12);
                        ...
                        if (this.dibujar == true) {
                            if (this.puntoInicio != null) {
                                ...
                                if (this.tipoFigura === 'rect') {
                                    p.rect(this.puntoInicio.x, this.puntoInicio.y, p.mouseX - this
                                        .puntoInicio.x, p.mouseY - this.puntoInicio.y);
                                    ...
                                    p.ellipse(this.puntoInicio.x + ((p.mouseX - this.puntoInicio.x) /
                                        2), this.puntoInicio.y + ((p.mouseY - this.puntoInicio
                                            .y) /
                                        2), (p.mouseX - this.puntoInicio.x), (p.mouseY - this
                                        .puntoInicio.y));
                                }
                            }
                        }
                        ...
                        if (this.figuraID != null) {
                            if (this.figuras[this.figuraID].selected == true) {
                                if (this.ctrlPresionado) {
                                    if (this.figuras[this.figuraID].type === 'rect' || this.figuras[this
                                            .figuraID].type === 'ellipse') {
                                        this.figuras[this.figuraID].updateMedidas(this.figuras[this
                                                .figuraID].x, this.figuras[this.figuraID].y, p
                                            .mouseX - this.figuras[this.figuraID].x, p.mouseY - this
                                            .figuras[this.figuraID].y);
                                    } else if (this.figuras[this.figuraID].type === 'line') {
                                        this.figuras[this.figuraID].updateLinea(this.figuras[this
                                                .figuraID].x1, this.figuras[this.figuraID].y1, p
                                            .mouseX, p.mouseY);
                                    }
                                }
                            }
                    ...
                    p.mouseClicked = () => {
                        if (this.dibujar == false) {
                            this.figuras.forEach((figura, index) => {
                                if (this.figuraSeleccionada == false && figura.hidden ==
                                    false) {
                                    if (figura.type !== 'text' && figura.type !== 'line') {
                                        let entra = false;
                                        if (figura.w >= 0 && figura.h >= 0) {
                                            if (p.mouseX >= figura.x && p.mouseX <= figura.x +
                                                figura.w && p.mouseY >= figura.y && p.mouseY <=
                                                figura.y + figura.h) {
                                                entra = true;
                                            }
                                        ...
                                        if (p.mouseX >= rectX2 && p.mouseX <= rectX2 +
                                            rectWidth2 && p.mouseY >= rectY2 && p.mouseY <=
                                            rectY2 + rectHeight2) {
                                            this.figuraSeleccionada = true;
                                            figura.selected = true;
                                            this.figuraID = index;
                    ...
                    p.mousePressed = () => {
                        if (this.dibujar && (p.mouseX > 0 && p.mouseX < p.width && p.mouseY > 0 && p
                                .mouseY < p.height)) {
                            if (this.tipoFigura === 'text' && this.tipoFigura !== '') {
                                ...
                                if (this.puntoInicio.length == 0) {
                                    ...
                                    if (this.tipoFigura === 'line') {
                                        this.figuras[0].updateLinea(this.puntoInicio.x, this.puntoInicio
                                            .y, this.puntoFinal.x, this.puntoFinal.y);
                                    } else {
                                        this.figuras[0].updateMedidas(this.puntoInicio.x, this
                                            .puntoInicio.y, this.puntoFinal.x - this.puntoInicio.x,
                                            this.puntoFinal.y - this.puntoInicio.y);
                                    }
                                    this.puntoInicio = [];
                                    this.puntoFinal = [];
                                }
                        ...
                        if (this.figuraID !== null && this.figuras[this.figuraID] !== undefined) {
                            if (this.figuras[this.figuraID].type !== 'text' && this.figuras[this
                                    .figuraID].type !== 'line') {
                                let entra = false;
                                if (this.figuras[this.figuraID].w >= 0 && this.figuras[this.figuraID]
                                    .h >= 0) {
                                    if (p.mouseX >= this.figuras[this.figuraID].x && p.mouseX <= this
                                        .figuras[this.figuraID].x + this.figuras[this.figuraID].w && p
                                        .mouseY >= this.figuras[this.figuraID].y && p.mouseY <= this
                                        .figuras[this.figuraID].y + this.figuras[this.figuraID].h) {
                                        entra = true;
                                    }
                                } else if (this.figuras[this.figuraID].w < 0 && this.figuras[this
                                        .figuraID].h >= 0) {
                                    if (p.mouseX <= this.figuras[this.figuraID].x && p.mouseX >= this
                                        .figuras[this.figuraID].x + this.figuras[this.figuraID].w && p
                                        .mouseY >= this.figuras[this.figuraID].y && p.mouseY <= this
                                ...
                                p.mouseX <= this.figuras[this.figuraID].x + this.figuras[this.figuraID]
                                .w &&
                                p.mouseY >= this.figuras[this.figuraID].y - this.figuras[this.figuraID]
                                .h &&
                                ...
                                let rectX3 = Math.min(this.figuras[this.figuraID].x1, this.figuras[this
                                    .figuraID].x2);
                                let rectY3 = Math.min(this.figuras[this.figuraID].y1, this.figuras[this
                                    .figuraID].y2);
                                let rectWidth3 = Math.abs(this.figuras[this.figuraID].x2 - this.figuras[
                                    this.figuraID].x1);
                                let rectHeight3 = Math.abs(this.figuras[this.figuraID].y2 - this
                                    .figuras[this.figuraID].y1);

                                if (p.mouseX >= rectX3 && p.mouseX <= rectX3 + rectWidth3 && p.mouseY >=
                                    rectY3 && p.mouseY <= rectY3 + rectHeight3) {
                    ...
                    p.mouseDragged = () => {
                        if (this.figuraID != null && this.figuras[this.figuraID] !== undefined) {
                            if (this.figuras[this.figuraID].type !== 'text' && this.figuras[this
                                ...
                                p.mouseY <= this.figuras[this.figuraID].y + 8 &&
                                this.figuras[this.figuraID].type === 'text') {
                                if (this.figuraID !== null && this.figuras[this.figuraID].hidden ==
                                    false) {
                                    this.figuras[this.figuraID].x = p.mouseX - offsetX;
                                    this.figuras[this.figuraID].y = p.mouseY - offsetY;
                                }
                            } else if (this.figuras[this.figuraID].type === 'line') {
                                let rectX4 = Math.min(this.figuras[this.figuraID].x1, this.figuras[this
                                    .figuraID].x2);
                                let rectY4 = Math.min(this.figuras[this.figuraID].y1, this.figuras[this
                                    .figuraID].y2);
                                let rectWidth4 = Math.abs(this.figuras[this.figuraID].x2 - this.figuras[
                                    this.figuraID].x1);
                                let rectHeight4 = Math.abs(this.figuras[this.figuraID].y2 - this
                                    .figuras[this.figuraID].y1);

                                if (p.mouseX >= rectX4 && p.mouseX <= rectX4 + rectWidth4 && p.mouseY >=
                                    rectY4 && p.mouseY <= rectY4 + rectHeight4) {
                                    if (this.figuraID !== null && this.figuras[this.figuraID].hidden ==
                                        false) {
                    ...
                    p.mouseReleased = () => {
                        if (this.figuraID != null && this.figuras[this.figuraID] !== undefined) {
                            if (this.figuras[this.figuraID].type !== 'text' && this.figuras[this
                                ...
                                if (entra == false && (p.mouseX > 0 && p.mouseX < p.width && p.mouseY >
                                        0 && p.mouseY < p.height)) {
                                ...
                                !(p.mouseX >= this.figuras[this.figuraID].x &&
                                    p.mouseX <= this.figuras[this.figuraID].x + this.figuras[this
                                        .figuraID].w &&
                                    p.mouseY >= this.figuras[this.figuraID].y - this.figuras[this
                                        .figuraID].h &&
                                    p.mouseY <= this.figuras[this.figuraID].y + 8) &&
                                this.figuras[this.figuraID].type === 'text') {
                                if (this.figuraID !== null && (p.mouseX > 0 && p.mouseX < p.width && p
                                        .mouseY > 0 && p.mouseY < p.height)) {
                                    this.figuras[this.figuraID].selected = false;
                                    this.figuraSeleccionada = false;
                                }
                            } else if (this.figuras[this.figuraID].type === 'line') {
                                let rectX5 = Math.min(this.figuras[this.figuraID].x1, this.figuras[this
                                    .figuraID].x2);
                                let rectY5 = Math.min(this.figuras[this.figuraID].y1, this.figuras[this
                                    .figuraID].y2);
                                let rectWidth5 = Math.abs(this.figuras[this.figuraID].x2 - this.figuras[
                                    this.figuraID].x1);
                                let rectHeight5 = Math.abs(this.figuras[this.figuraID].y2 - this
                                    .figuras[this.figuraID].y1);
